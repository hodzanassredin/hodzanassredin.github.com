...
Иногда в приложениях нам необходимо создавать рабочие процессы. Исполнение которых занимает продолжительное время. Например после создания документа отправить письмо менеджеру с ссылкой на подтверждение публикации. Это самый простой пример, в реальном мире рабочие процессы могут быть чрезвычайно сложны. В веб приложениях оипичный пример при оформление покупок в интернет магазине. На рынке существуют различные готовые решения этой проблемы например Microsoft Workflow Foundation. Но для некоторых задач это явный оверкил. В большинстве приложений не требуется позволять пользователям создавать и редактировать свои кастомные процессы. Давайте же попробуем написать свое легковесное решение. Для начала определим список требований.
...
5. Сохранение и сериализация Процесса должно быть легко реализуемо.
...
{% highlight csharp %}
var a = Ask<int> ("enter a");
//проверить если уже существует результат выполнение активности
...
var b = Ask<int> ("enter b");
//Поведение тоже но линия зависит от предыдущего результата "a"
...
{% endhighlight %}
Мы все ще не знаем как написать функции IsExecuted и GetResult класса WokflowStep. Мы должны где то сохранять результаты уже выполненных активностей. Мы можем инкрементно присваивать номера линий на каждое создание активностей и использовать List<String> как хранилище сериализованных результатов. Мы будем использовать класс ExecutionСontext который будет использован как харнилище результатов и текущего номера исполняемой строки.
{% highlight csharp %}
