...
## {{page.title}} [RUS]({{ site.url }}/2014/07/02/2014-07-02-way-to-computation-expressions-rus.html)

<p class="meta">02 July 2014 &#8211; Karelia</p>

In the last [post]({{ site.url }}/2014/06/21/yet-another-monad-guide-rus.html) we described, in short, what monads ishow we can represent them in c#. Now lets check some limitations and possible solutions of our monads implementation.
So monads alows us to describe some abstract computations line by line and run them on top of container types which implemnts our IMonad interface. Also in c# we can use syntatic shugar which allows us to express that computations in more convinient way.
In fact a monad is a computer, and described computations is a program for it. Hence the question, is our monadic computer are turing complete? Could we express any lghorithm in that monadic language? And the answer is yes. Without this property IO monad in haskell could not imitate any possible way of computation with side effects. But lets prove that monads are turing complete by writing some code. Lets try to describe usual control flow expressions: "if else" and "while do" for IMonad interface and describe them in terms of bind and return members.
{% highlight csharp %}
