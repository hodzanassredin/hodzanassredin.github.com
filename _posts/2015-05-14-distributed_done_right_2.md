...
Consumer could write into the queue when it is not full in other case it will block. Producer could read from the queue when it is not empty and in other case it will block. The queue is buffered to increase overall speed and in case then there is multiple consumers and producers increase throughput. When queue is safe to use by multiple consumers and producers then it could be easy to change level of concurrency for different parts of pipeline in runtime.
We will use BlockingQueueAgent described by Thomas Patricheck [here](http://tomasp.net/blog/parallel-extra-blockingagent.aspx/) next samples will be almost the same as in this [post](http://tomasp.net/blog/parallel-extra-image-pipeline.aspx/) but with some additions. So you could go and read these awesome posts and after that return back. But I'll describe here in short just to keep all info in one place. We should describe our pipeline as separate tasks.  
{% highlight fsharp %}
