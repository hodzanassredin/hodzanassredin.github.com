...
It is always interesting to start from the base things and try to rethink them from the other point of view. So, I started to think about zero and sum and other basic things in terms of programming. And this blog post is an experiment to write about math and replace formulas by programs.

# The numbers

Let’s start from the number. Initially we have only nothing(zero) 
...
We could use idea of lazy lists but without values.
...
But actually, we could use fold and unfold to express them. Fold function will iterate a number and create an aggregate.
...
Now we are ready to start with arithmetic. But even simple decrement from zero is not supported in our numbers.
We need to return optional answer in case of unsupported operation. Probably in future 
...
One interesting thing to note I expected to see unfold in subtraction, but it is fold probably because of unsupported negative numbers.
We can see sum as increment in a for loop. 
And minus to find argument of the sum function with a given result and argument.
...
Now we are ready for more advanced functions and they look interesting, we can see some patterns.
Unfortunately, we do not support rational numbers so we implement division as a function which will return 
how many times (rounded to integer) numerator contains denominator.
...
It was a fun to see that arithmetic looks so interesting in a functional manner.

Next steps will be to support all real numbers (including irrational) 
and root function.

One interesting thing is about div by zero. 
If we delete any number (even zero) by zero, we will have infinite sequence as a result.
It is a great start to check different kinds of infinity and to add infinity detector into fold unfold functions.
For example, we can keep whole history of a state and try to find current state in the state history.
And extend our number with Infinity case. We will be able to express rational numbers. 
Unfortunately, it will not work for irrational numbers but who knows about other ways to check for infinity. Probably we can express them as continued fraction.
